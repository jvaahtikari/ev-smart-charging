...
A fully automated EV charging scheduler for Home Assistant that optimises charging cost using real-time spot prices and 12-hour ahead Nordpool forecasts. Built for a **Smart #1 car** and **Zaptec Pro charger**, but the logic is adaptable to any EV + charger combination.
...
| **12 h price forecast** | Extends planning beyond spot-price availability using Nordpool Predict FI |
...
| Nordpool Predict FI | `sensor.nordpool_predict_fi_*` | Custom integration for 12 h price forecast |
...
3. MIN CHARGE   ev_min_charge_enabled = on  AND  current slot in min-charge plan
                → picks cheapest slots within [now … min(min_deadline, horizon)]
...
config/
  packages/
    ev_effective_predict_and_control.yaml   ← master control + Charge Now + Min Charge
  ...
  configuration.yaml                        ← sensor.ev_schedule_check (lines ~150-260)
  automations.yaml                          ← ev_plug_in_takeover_gate, ev_target_reached_stop,
                                               ev_campaign_cost_*, ev_price_slots_15m_refresh, …
...
| `input_boolean.ev_allow_predicted_hours` | on | Use 12 h Nordpool forecast |
...
| `input_datetime.ev_deadline` | — | Smart plan deadline |
...
| `input_number.ev_min_charge_by_hour` | 8 | Daily hour (0–23) for min-charge deadline |
...
Nordpool spot (15 min slots)  ──┐
                                ├─► ev_price_slots_15m_effective  ──► ev_plan_15m_rank
Nordpool Predict FI (12 h)    ──┘   (merged, globally ranked)         (deadline-bounded
  └─► ev_predict_15m_series                                            cheapest N slots)
```

When `ev_allow_predicted_hours = on`, predicted slots beyond the spot horizon are included in planning. The effective price series is globally ranked so predicted and spot slots compete on equal footing.

The planning horizon is `min(spot_horizon, 12 h)` from now (quantised to 15-min boundaries).
