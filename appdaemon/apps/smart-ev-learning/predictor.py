...
from utils import ha_write  # Session 2: shared write utility
...
class EVPredictor(hass.Hass):
    ...
    def _publish(self, target, confidence, prediction_active, status,
        ...
        # Session 2: write ev_last_target_decision via input_text backend
        # (sensor.ev_last_target_decision template reads from this input_text)
        profile_name = (
            f"{get_season(datetime.now().month)}|"
            f"{get_temp_band(round(temp, 1))}|mixed|short|"
            f"{'preheated' if preheating else 'cold_start'}"
        ) if not fallback_used else f"fallback:{fallback_reason}"

        ha_write(
            self,
            "input_text.ev_last_target_decision",
            "ok" if not fallback_used else "fallback",
            {
                "friendly_name":    "EV Last Target Decision (backend)",
                "decision_time":    datetime.now().isoformat(),
                "mode":             "fallback" if fallback_used else "ml",
                "profile_name":     profile_name,
                "profile_trips":    0 if fallback_used else 0,  # updated per profile below
                "bargain_triggered": False,
                "window_min_price": 0,
                "target_kwh":       round(target / 100.0 * self.battery_kwh, 2),
